...
function MachinesIndex(index_options, machines, loader, mdialogs) {
    ...
    document.getElementById("filter-menus").addEventListener("change", manage_search);
    ...
    function update_sidebar(machine, state, compiled) {
        ...
        function keyword_relevance(current_best, item) {
            const native = item.native || false;
            const coef = item.coef || 0;
            let score;
            let _m = "";
            let best = { score: -1 };
        ...
        function keyword_filter(item) {
            if (!term)
                return true;
            let best_keyword = Object.values(item.keywords).reduce(keyword_relevance, { score:-1 });
            if (best_keyword.score === -1 && page_contains && page_contains[machine.key] && page_contains[machine.key][item.path]) {
                best_keyword = Object.values(page_contains[machine.key][item.path]).reduce(keyword_relevance, { score:-1 });
            }
