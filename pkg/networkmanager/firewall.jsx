...
class AddServicesModal extends React.Component {
    ...
    validate(event) {
        ...
        ports.forEach(function(port) {
            ...
            if (!port)
                return;
            let p = self.state.avail_services[port.trim()];
            if (!p) {
                let num_p = Number(port);
                if (isNaN(num_p))
                    error = _("Unknown service name");
                else if (num_p <= 0 || num_p > 65535)
                    error = _("Not valid port number");
                else
                    new_ports.push(port);
            } else if (p['type'].indexOf(targets[0]) < 0)
                error = _("Port number and type do not match");
            else {
                new_ports.push(p.port);
            }
        });
                    ...
                    <Modal.Body id="add-services-dialog">
                        <form action="" className="toggle-body ct-form-layout">
                            <label className="radio ct-form-layout-full">
                                <input type="radio" name="type" value="services" onClick={this.onToggleType} checked={!this.state.custom} />
                                {_("Services")}
                            </label>
                            { this.state.custom ||
                                <React.Fragment>
                            ...
                            <div />
                            <label className="radio ct-form-layout-full">
                                <input type="radio" name="type" value="ports" onClick={this.onToggleType} checked={this.state.custom} disabled={this.state.avail_services == null} />
                                {_("Custom Ports")}
                            </label>
                            { !this.state.custom ||
                                <form className="ct-form-layout">
                                    ...
                                    <p id="hint">
                                        {_("Comma-separated ports, ranges and alliases are accepted")}
                                    </p>
                                    ...
                                    <span className="error-msg">{this.state.tcp_error}</span>
                                    ...
                                    <span className="error-msg">{this.state.udp_error}</span>
